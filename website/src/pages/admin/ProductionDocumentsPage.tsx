...
import { ChevronDown, ChevronRight, Eye, Printer, Search } from 'lucide-react';
...
function normalizeItems(value: unknown): TransaksiItem[] {
  ...
  return value
    .map((raw) => {
      ...
      return { productId, productName, ukuran, warna, qty: Number.isFinite(qty) ? qty : 0 };
    })
...
<body>
  <div class="page">
    ...
    <div class="section row2">
      ...
      <div class="block">
        <div class="block-title">KETERANGAN</div>
        <table>
          <thead>
            <tr>
              <th class="cell head center" style="width:22%;">Atasan Size</th>
              <th class="cell head center" style="width:18%;">Atasan Qty</th>
              <th class="cell head center" style="width:22%;">Pants Size</th>
              <th class="cell head center" style="width:18%;">Pants Qty</th>
              <th class="cell head center" style="width:20%;">Keterangan</th>
            </tr>
          </thead>
          <tbody>
            ${sizeRowsHtml}
            <tr>
              <td class="cell head center" colspan="1"><b>Total</b></td>
              <td class="cell head center" colspan="4"><b>${escapeHtml(String(totalQty))}</b></td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="section materials">
      <div class="block">
        ...
        <div class="mat-body">
          <div class="mat-grid">
            <div>Kerah</div><div>:</div><div><b>${kerah}</b></div>
            <div>Body</div><div>:</div><div><b>${body}</b></div>
            <div>Aplikasi</div><div>:</div><div><b>${aplikasi}</b></div>
            <div>Lengan</div><div>:</div><div><b>${lengan}</b></div>
            <div>Celana</div><div>:</div><div><b>${celana}</b></div>
          </div>
        </div>
      </div>

      <div class="block">
        <div class="block-title">LOGO</div>
        <div class="mat-right">
          <img src="${escapeHtml(logoUrl)}" alt="Logo" style="max-width: 150px; max-height: 60px; object-fit: contain;" onerror="this.style.display='none'" />
        </div>
      </div>
    </div>
...
export default function ProductionDocumentsPage() {
  ...
  const listQuery = useQuery({
    queryKey: ['transaksi', 'dokumen-spk'],
    queryFn: async (): Promise<SpkRow[]> => {
      ...
      for (const t of transaksis) {
        ...
        for (let i = 0; i < groups.length; i++) {
          ...
          const stepStatus = coerceSpkStepStatus(detail?.stepStatus) ?? 'DESIGN';
  ...
  const groupedByInvoice = useMemo(() => {
    ...
    return Array.from(groups.entries())
      .map(([invoice, rows]) => {
        ...
        const doneCount = sortedRows.reduce((acc, r) => (r.stepStatus === 'SELESAI' ? acc + 1 : acc), 0);
        return { invoice, rows: sortedRows, latestOrderDateMs, doneCount };
      })
      .filter((g) => g.rows.length > 0 && g.doneCount === g.rows.length)
      .sort((a, b) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">Dokumen (SPK Done)</h1>
      </div>
                ...
                return (
                  <Fragment key={g.invoice}>
                    <TableRow className="cursor-pointer" onClick={() => toggleInvoice(g.invoice)}>
                      ...
                      <TableCell className="text-right text-muted-foreground">{`${g.doneCount}/${g.rows.length} Selesai`}</TableCell>
                    </TableRow>
                              ...
                              <TableBody>
                                {g.rows.map((row, rowIdx) => (
                                  <TableRow key={row.id}>
                                    ...
                                    <TableCell className="text-velcrone-success">SELESAI</TableCell>
      ...
      <Dialog open={!!selectedDetail} onOpenChange={(open) => !open && setSelectedDetail(null)}>
        <DialogContent className="bg-card max-w-2xl">
          <DialogHeader><DialogTitle>Detail Dokumen SPK</DialogTitle></DialogHeader>
          {selectedDetail && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="sm:col-span-2">
                  <p className="text-sm text-muted-foreground">Jenis - Qty</p>
                ...
                <Table>
                  ...
                  <TableBody>
                    {selectedDetail.sizes.length === 0 ? (
                      ...
                      selectedDetail.sizes.map((it, i) => (
                        <TableRow key={`${it.size}-${i}`}>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={() => void previewSpk(selectedDetail)} className="velcrone-gradient text-primary-foreground hover:opacity-90">
                  <Printer size={16} className="mr-2" /> Print SPK
                </Button>
              </div>
            </div>
          )}
